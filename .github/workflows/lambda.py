...
def validate_key_pair(private_key, public_key):
    ...
    try:
        ...
        print("Public key matches the private key.")
    except Exception as e:
        print("Public key does not match the private key.")
...
def rotate_secret(secret_name):
    ...
    # client.put_secret_value(SecretId=secret_name, SecretString=json.dumps({"new_secret_key": new_secret_value}))

    # Replace 'your-secret-name' with the name of your secret in Secrets Manager
    # rotate_secret('your-secret-name')
...
def create_secret(service_client, arn, token, context):
    ...
    try:
        ...
        [priv, pub] = rotate_secret(secret_name)

        current_dict['PRIVATE_KEY'] = priv
        current_dict['PUBLIC_KEY'] = pub
        validate_key_pair(priv, pub)
...
def set_secret(service_client, arn, token, context):
    print(" --------- Inside set-secrets --------------")
    print(service_client, arn, token, context)
    """Set the secret
...
def test_secret(service_client, arn, token, context):
    ...
    print("testSecret: Performing SSH test by invoking command {service_client} {arn} {token} {context}")
